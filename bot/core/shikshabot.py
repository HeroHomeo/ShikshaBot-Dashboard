...
class shikshabot(commands.AutoShardedBot):
    ...
    async def load_extensions(self):
        for extension in extensions:
            try:
                ...
                import traceback
                traceback.print_exc()
                print(f"{Fore.RED}{Style.BRIGHT}Failed to load extension {extension}. {type(e).__name__}: {e}")
...
def setup_bot():
    intents = discord.Intents.all()
    bot = shikshabot(intents=intents)
    return bot
